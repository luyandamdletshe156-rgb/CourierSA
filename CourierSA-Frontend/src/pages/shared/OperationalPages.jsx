    ...
    <AppShell title="Delivery History">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">All completed and failed deliveries</p>
        </div>
      </div>
      ...
      <div className="card">
        {isLoading ? <PageLoader /> : deliveries.length === 0 ? (
          <EmptyState icon={Truck} title="No delivery history" description="Completed deliveries will appear here." />
        ) : (
          <>
            <div className="table-container">
              <table className="table">
                ...
                <tbody>
                  {deliveries.map(d => (
                    <tr key={d.id}>
                      <td><TrackingBadge value={d.trackingNumber} /></td>
                      <td className="text-sm font-medium text-[#172554]">{d.recipientName}</td>
                      <td className="text-xs text-[#64748B]">{d.city}</td>
