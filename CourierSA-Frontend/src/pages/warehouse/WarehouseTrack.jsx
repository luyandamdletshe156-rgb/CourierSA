    ...
    <AppShell title="Track Parcel">
      ...
      <div className="card mb-6">
        <div className="flex gap-2">
          <input type="text" className="input font-mono flex-1" placeholder="Enter tracking number…" value={trackingNumber} onChange={e => setTrackingNumber(e.target.value.toUpperCase())} />
          <button className="btn-primary" onClick={() => setSearched(trackingNumber.trim())} disabled={!trackingNumber.trim()}><Search size={15} /> Track</button>
        </div>
      </div>
      ...
      {result && (
        <div className="card">
          <div className="card-header"><div className="flex items-center gap-3"><TrackingBadge value={result.trackingNumber} /><StatusPill status={result.status} /></div></div>
          <p className="text-xs font-bold text-[#172554] mb-3 flex items-center gap-1.5"><MapPin size={13} /> Tracking history</p>
