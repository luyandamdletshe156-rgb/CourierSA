import { Package, Trash2, MapPin, Flame } from 'lucide-react'
        ...
        {items.map((item, i) => (
          <div key={item.cartItemId} className="flex items-start justify-between gap-3 px-4 py-3 rounded-xl border border-[#D8E4F5] bg-[#F6FAFF]">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[#94A3B8] mt-0.5 truncate">{item.description} · {item.weightKg} kg</p>
              {item.scheduledPickupDate && (
                <p className="text-xs text-[#94A3B8] mt-0.5">Scheduled: {item.scheduledPickupDate}</p>
              )}
            </div>
            <div className="flex flex-col items-end gap-2 flex-shrink-0">
              ...
              <button
                ...
                aria-label={`Remove parcel ${i + 1} from cart`}
              >
                <Trash2 size={15} />
              </button>
            </div>
